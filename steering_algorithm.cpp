...
// Framework: https://github.com/chalmers-revere/working-with-rec-files/tree/master/cpp-opencv
                ...
                // Pick the largest valid contour on the requested side and return its horizontal center (-1 if none found).
                auto findBiggestConeX = [&](std::vector<std::vector<cv::Point>> const& shapes,
                                            double minArea,
                                            bool leftSide) -> double {
                    double bestX = -1.0;
                    ...
                    for (auto const& shape : shapes) {
                        ...
                        if (size > minArea && size < 5000) {
                            cv::Rect box = cv::boundingRect(shape);
                            double boxCenter = box.x + box.width / 2.0;

                            bool onCorrectSide = leftSide ? (boxCenter < centerOfScreen)
                                                          : (boxCenter > centerOfScreen);
                            if (onCorrectSide && size > biggestSize) {
                                biggestSize = size;
                                bestX = boxCenter;
                            }
                        }
                ...
                double bestBlueX = findBiggestConeX(blueShapes, 40.0, true);
                ...
                double bestYellowX = findBiggestConeX(yellowShapes, 100.0, false);
